...
function Checkout() {
  ...
  const onDragEnd = (e) => {
    console.log("e---", e.target._latlng);
    dispatch(
  ...
  const getAddressByLatLng = async (latitude, longitude) => {
    try {
      ...
      console.log("getAddressByLatLng error: ", e);
    }
  };
  ...
  const getLatLngByAddress = async () => {
    try {
      if (addressInput) {
        const { data } = await axios.get(
          `https://api.geoapify.com/v1/geocode/search?text=${encodeURIComponent(addressInput)}&format=json&apiKey=${import.meta.env.VITE_GEO_API_KEY}`,
        );
        dispatch(
          setLocation({ lat: data.results[0].lat, long: data.results[0].lon }),
        );
        getAddressByLatLng(data.results[0].lat, data.results[0].lon);
      }
    } catch (e) {
      console.log("getLatLngByAddress error: ", e);
    }
  };
      ...
      <div className="w-full max-w-[900px] bg-white rounded-2xl shadow-xl p-6 space-y-6">
        ...
        <section>
          ...
          <div className="flex gap-2 mb-3">
            ...
            />
            <button
              className="bg-[#ff4d2d] hover:bg-[#e64526] text-white px-3 py-2
            ...
            <button
              className="bg-blue-500 hover:bg-blue-600 text-white text-white px-3 py-2
                    rounded-lg flex items-center justify-center cursor-pointer"
          ...
          <div className="rounded-xl border overflow-hidden">
            <div className="h-64 w-full flex items-center justify-cneter">
              <MapContainer
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div
              ...
              className={`flex items-center gap-3 rounded-xl border p-4 text-left cursor-pointer
                        transition ${paymentMethod == "cod" ? "border-[#ff4d2d] bg-orange-50 shadow" : "border-gray-200 hover:border-gray-300"}`}
            >
              <span
            ...
            <div
              ...
              className={`flex items-center gap-3 rounded-xl border p-4 text-left cursor-pointer
                        transition ${paymentMethod == "online" ? "border-[#ff4d2d] bg-orange-50 shadow" : "border-gray-200 hover:border-gray-300"}`}
            >
              <span
                className="inline-flex h-10 w-10 items-center justify-center
        ...
        font-semibold cursor-pointer">{paymentMethod == "cod" ? "Place Order": "Pay & Place Order"}</button>
      </div>
    </div>
  );
}
